...
provider "azurerm" {
  features {}
  subscription_id = "60a78dcf-a3f5-4f8a-a795-68985b21f3f0"
}
...
resource "azurerm_service_plan" "arp" {
...
resource "azurerm_linux_web_app" "alwa" {
  ...
  resource_group_name = azurerm_resource_group.arg.name
  location            = azurerm_service_plan.arp.location
  service_plan_id     = azurerm_service_plan.arp.id
...
resource "azurerm_app_service_source_control" "asc" {
  ...
  repo_url               = "https://github.com/GalinaUzunova/task-board"
  branch                 = "main"
